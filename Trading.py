...
end_date_supertrend = dt.datetime(2050, 7,1) 
...
condition2 = { "scan_clause": "( {cash} ( ( {cash} ( latest supertrend( 21,1 ) <= latest close and latest supertrend( 14,2 ) <= latest close and latest supertrend( 7 , 2 ) < latest close and 1 day ago  supertrend( 7 , 3 ) >= 1 day ago  close and latest volume > 200000 and latest close - 1 candle ago close / 1 candle ago close * 100 <= 5 and latest close >= 50 and latest close > latest ema( latest close , 22 ) and latest close > latest ema( latest close , 44 ) and latest close >= latest ema( latest close , 100 ) ) ) ) )" }
...
with requests.session() as s:
    ...
    name_array1 = stock_list1['nsecode'].apply(lambda x: x + '.NS').values

    print(name_array1,"lop")
    ...
    # name_array2 = stock_list2['nsecode'].apply(lambda x: x + '.NS').values

    # print(name_array2,"lop")
    data3 = s.post(url, headers=header, data=condition3).json()
    stock_list3 = pd.DataFrame(data3["data"])
    name_array3 = stock_list3['nsecode'].apply(lambda x: x + '.NS').values
    print(name_array3,"kom")
...
def supertrend1(stock_symbol, start_date, end_date_supertrend):
    stock_data = yf.download(stock_symbol, start=start_date, end=end_date_supertrend)
    print(stock_data,"lpo")
    if stock_data.empty:
    ...
    stock_data.ta.supertrend(length=supertrend_period, multiplier=multiplier, append=True)

    column_name = f"SUPERT_{supertrend_period}_{multiplier}"
    if column_name not in stock_data.columns:
        print(f"⚠️ Supertrend column not found in data for {stock_symbol}")
        return None

    latest_value = stock_data[column_name].dropna().iloc[-1]
    return float(latest_value)
def supertrend2(stock_symbol, start_date, end_date_supertrend):
    stock_data = yf.download(stock_symbol, start=start_date, end=end_date_supertrend)
    ...
    latest_value = stock_data[column_name].dropna().iloc[-1]
    return float(latest_value)
def supertrend3(stock_symbol, start_date, end_date_supertrend):
    stock_data = yf.download(stock_symbol, start=start_date, end=end_date_supertrend)
...
target2_array = []
rr_array = []
for symbol in tqdm(name_array1, desc="Scanning Array 1", miniters=1):
    ...
    if(open<close):
        if(hc<=co):
            st1 = supertrend1(symbol, start_date, end_date_supertrend)
            st2 = supertrend2(symbol, start_date, end_date_supertrend)
            st3 = supertrend3(symbol, start_date, end_date_supertrend)
...
# for symbol in tqdm(name_array2, desc="Scanning Array 2", miniters=1):
#     open = get_opening_price(symbol)
#     close = get_closing_price(symbol)
#     high = get_high_price(symbol)
#     low= get_low_price(symbol)
...
# for symbol in tqdm(name_array3, desc="Scanning Array 3", miniters=1):
#     open = get_opening_price(symbol)
#     close = get_closing_price(symbol)
#     high = get_high_price(symbol)
#     low= get_low_price(symbol)
